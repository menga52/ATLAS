  ...
  for i in range(K):
    if i in index_to_learn:
      ...
      while (relative_error[0]>relative_threshold[0] or relative_error[1]>relative_threshold[1]) \
        ...
        relative_error[0] = np.linalg.norm(chart[i].sigma - chart_new.sigma,2)/np.linalg.norm(chart_new.sigma,2)
        relative_error[1] = np.linalg.norm(chart[i].b-chart_new.b,2)/np.linalg.norm(chart_new.b,2)
        if verbose:

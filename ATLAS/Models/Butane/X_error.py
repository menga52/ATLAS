...
def X_error(chart_entry, parameter):
  ...
  sigma_abs_error = np.norm(np.norm(H_true)-abs(sigma))
  sigma_error = sigma_abs_error/np.norm(H_true) # norm(H_true, 'fro')
  ...
  b_abs_error = np.norm(b-b_true)
  b_error = b_abs_error/np.norm(b_true)

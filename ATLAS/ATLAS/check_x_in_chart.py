...
def check_x_in_chart(x, chart, t0, r, chi_p, threshold):
  ...
  Center      = X_int
  x_coeff     = (x - Center) @ WU
  if  sum(x_coeff**2 / sigma**2) <= r**2 * t0 * chi_p and (np.linalg.norm(x-X_int)<threshold )   # this is  an very ad-hoc threshold. which is determined by the thickness of fast mode
